...
Codex Micro Addons launches the installed Codex desktop app with Electron debugging enabled on a random port bound to `127.0.0.1`. The sidecar evaluates only the entrypoints present under the locally built helper's `Contents/Resources/addons/` directory.
...
- `focus-thread-window` declares only `focus-codex-window`. That action runs the fixed command `/usr/bin/open -b com.openai.codex`; no payload text is passed into the command.
